...
export class CoinGeckoService {
  ...
  async getTokenPrices(
    ...
    try {
      ...
      const tokensWithoutPrices = tokenAddresses.filter((addr) => !prices[addr.toLowerCase()])

      if (tokensWithoutPrices.length > 0) {
        console.debug(
          `No prices found for ${tokensWithoutPrices.length} tokens (not on CoinGecko):`,
          tokensWithoutPrices,
        )
      }

      console.debug(
        `Parsed prices for ${Object.keys(prices).length} out of ${tokenAddresses.length} tokens`,
      )
  ...
  private async fetchTokenPricesInBatches(
    ...
    const batchPromises = batches.map(async (batch, index) => {
      ...
      try {
        ...
        console.debug(
          `Batch ${index + 1}/${batches.length} returned ${Object.keys(batchPrices).length} prices`,
        )
        return batchPrices
      } catch (error) {
    ...
    console.debug(
      `Total prices fetched: ${Object.keys(allPrices).length} out of ${sortedAddresses.length}`,
    )

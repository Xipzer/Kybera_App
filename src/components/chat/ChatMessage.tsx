...
import { parseUiBlocks, stripUiBlocks, stripJsonActionBlocks, UiBlockCard } from './ui-blocks'
...
function stripAgentBlocks(text: string): string {
  return stripUiBlocks(
    stripJsonActionBlocks(text)
  )
}
...
export function ChatMessage({ message }: ChatMessageProps) {
  ...
  // Parse UI blocks from assistant messages
  const uiBlocks = useMemo(() => {
    if (message.role !== 'assistant') return []
    return parseUiBlocks(message.content ?? '')
  }, [message.content, message.role])

  const renderedContent = useMemo(() => {
    const content = typeof message.content === 'string' ? message.content : ''
    if (message.role === 'assistant') {
      // Strip JSON action blocks and kybera-ui blocks before rendering markdown
      const cleaned = stripAgentBlocks(content)
      return cleaned ? md.render(cleaned) : ''
    }
    return content
  }, [message.content, message.role])
  ...
  // Don't render empty assistant messages (all content was stripped as JSON/UI blocks)
  const hasContent = renderedContent.trim().length > 0
  const hasBlocks = uiBlocks.length > 0
  if (!isUser && !hasContent && !hasBlocks) return null
  ...
  return (
    <div className={`flex gap-3 mb-6 ${isUser ? 'justify-end' : ''}`}>
      ...
      <div className={`relative max-w-[85%] sm:max-w-[75%] group space-y-2`}>
        {isUser ? (
          <div
          ...
          <>
            {/* Text bubble — only if there's prose to show */}
            {hasContent && (
              <div
            ...
            {uiBlocks.length > 0 && (
              <div className="space-y-2">
                {uiBlocks.map((block, i) => (
                  <UiBlockCard key={`ui-${block.type}-${i}`} block={block} />
                ))}
              </div>
            )}
          </>

...
export function ChatSidebar({ collapsed }: ChatSidebarProps) {
  ...
  return (
    <div className="h-full flex flex-col panel-content-fade">
      <div className={`p-3 sm:p-4 ${styles.headerBg} border-b ${styles.headerBorder}`}>
        <div className="flex items-center justify-between mb-3 sm:mb-4">
          <div className="flex items-center gap-1.5 sm:gap-2">
            <div className={`p-1 sm:p-1.5 rounded-lg bg-gradient-to-r ${styles.newChatGradient}`}>
        ...
        <button onClick={handleNewChat} className={`relative w-full group overflow-hidden`}>
          <div
            className={`flex items-center justify-center gap-1.5 sm:gap-2 px-3 sm:px-4 py-2 sm:py-2.5 bg-gradient-to-r ${styles.newChatGradient} rounded-xl font-medium text-white text-sm sm:text-base shadow-lg ${styles.newChatShadow} transition-all duration-300 hover:shadow-xl hover:scale-[1.02] active:scale-[0.98]`}
          >
            ...
            {sortedConversations.map((conversation) => {
              ...
              return (
                ...
                >
                  {isActive && (
                    <div
                      className={`absolute left-0 top-1/2 -translate-y-1/2 w-1 h-6 sm:h-8 rounded-r-full bg-gradient-to-b ${styles.newChatGradient}`}
                    />
                  )}
                  ...
                  <div className="flex items-center gap-2 sm:gap-3 min-w-0 pl-1">
                    ...
                    >
                      {conversation.pinned ? (
                        <Pin
                          className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${isActive ? 'text-white' : styles.iconActive}`}
                        />
                      ) : (
                        <MessageSquare
                          className={`w-3 h-3 sm:w-3.5 sm:h-3.5 ${isActive ? 'text-white' : styles.iconDefault}`}
                        />
                      )}
                    </div>
                    ...
                    <div className="min-w-0 flex-1">
                      {renamingId === conversation.id ? (
                        <input
                          ...
                          className={`w-full px-2 py-1 text-xs sm:text-sm ${styles.inputBg} border ${styles.inputBorder} rounded-lg focus:outline-none focus:ring-2 ${styles.inputFocus} text-text-primary transition-all`}
                          style={{ fontSize: '16px' }}
                          autoFocus
                        />
                      ) : (
                        <>
                          <p className="text-xs sm:text-sm font-medium truncate text-text-primary">
                            {conversation.title}
                          </p>

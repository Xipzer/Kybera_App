...
export function SendDialog({
  ...
  useEffect(() => {
    const fetchBalances = async () => {
      ...
      // Check for network type mismatch before fetching
      if (fromWallet.type !== network.type) {
        console.log('SendDialog: Wallet type mismatch, skipping balance fetch', {
        ...
        >
          <div className="p-6">
            ...
            <div className="space-y-6">
              {/* From Section */}
              <div>
                <label className={`${theme.styles.label} mb-3 block`}>From</label>
                ...
                >
                  <Select.Trigger
                    className={`w-full flex items-center justify-between p-3 ${theme.styles.input} hover:border-border-default transition-colors`}
                  >
                    ...
                    <Popover.Trigger asChild>
                      ...
                      >
                        <div className="w-8 h-8 bg-surface-elevated rounded-full flex items-center justify-center overflow-hidden flex-shrink-0">
                          ...
                          <span
                            className={`text-sm font-medium ${selectedToken?.logoURI ? 'hidden' : ''}`}
                          >
                              ...
                              return filteredTokens.map((token) => {
                                ...
                                return (
                                  ...
                                  >
                                    <div className="flex items-center gap-3">
                                      <div
                                        className={`w-8 h-8 bg-surface-elevated rounded-full flex items-center justify-center overflow-hidden`}
                                      >
                                        {token.logoURI ? (
                                          <img
                                            src={token.logoURI}
                                        ...
                                        <span
                                          className={`text-sm font-medium ${token.logoURI ? 'hidden' : ''}`}
                                        >
              ...
              {/* To Section */}
              <div>
                <label className={`${theme.styles.label} mb-3 block`}>To</label>
                ...
                <div className="mt-3">
                  <Tabs.Root value={activeTab} onValueChange={setActiveTab}>
                    ...
                    <Tabs.Content value="wallets" className="mt-3">
                      <div
                        className={`${theme.styles.dropdown?.content || 'bg-surface-base border border-border-subtle rounded-lg'} overflow-hidden`}
                      >
                        {/* Search Bar */}
                        <div className="p-3 border-b border-border-subtle">
                          <div className="relative">
                              ...
                              .map(({ group, wallets }) => ({
                                ...
                                wallets: wallets.filter(
                                  (wallet) =>
                                    wallet.name
                                      .toLowerCase()
                                      .includes(walletSearchQuery.toLowerCase()) ||
                                    wallet.address
                                      .toLowerCase()
                                      .includes(walletSearchQuery.toLowerCase()),
                                ),
              ...
              {network.type === 'SVM' && (
                <div>
                  <label className={theme.styles.label}>Memo (Optional)</label>
                  <input
                    type="text"
                    value={memo}
              ...
              <div className="pt-2">
                <button
                  onClick={handleSend}
                  disabled={
                    isLoading || !recipient || !amount || !selectedToken || isLoadingBalances
                  }

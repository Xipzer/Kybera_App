...
function RiskBadge({ level }: { level: string }) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1 px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full text-2xs sm:text-xs font-semibold uppercase tracking-wider ${config.bg} ${config.text} border ${config.border}`}
    >
  ...
  return (
    <Popover.Root open={open} onOpenChange={setOpen}>
      <Popover.Trigger asChild>
        <button
          className={`flex items-center gap-1.5 px-2 py-1 rounded-lg transition-colors bg-surface-elevated ${theme.styles.listItemHover}`}
        >
          {selected.value ? (
          ...
          <span className={`text-[11px] font-medium ${theme.styles.textPrimary}`}>
            {selected.label}
          </span>
          <ChevronDown
            className={`w-3 h-3 ${theme.styles.iconSecondary} transition-transform ${open ? 'rotate-180' : ''}`}
          />
        </button>
      </Popover.Trigger>
      <Popover.Portal>
              ...
              >
                <div className="flex items-center justify-between w-full">
                  <div className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-xs ${networkFilter === n.value ? 'text-accent font-medium' : theme.styles.textPrimary}`}
                    >
                      {n.label}
                    </span>
                  </div>
                ...
                <div className="p-3 sm:p-4">
                  <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 sm:gap-4">
                    <div>
                      ...
                      {opp.apyReward > 0 && (
                        <div className="text-2xs sm:text-xs text-text-tertiary mt-0.5">
                    ...
                    <div className="col-span-2 sm:col-span-1">
                      ...
                      <div className="text-xs sm:text-base font-medium text-text-secondary capitalize">
                        {opp.yieldType.replace('_', ' ')}
                      </div>
                    </div>
                  </div>
                </div>
                    ...
                    <div className={`${tc.sectionBg} border rounded-xl p-2.5 sm:p-3`}>
                      <div className="flex flex-wrap gap-1.5">
                        {opp.riskFactors.map((factor, i) => (
                          <span
                            key={i}
                            className="px-2 py-0.5 rounded-full text-2xs sm:text-xs font-medium bg-orange-500/20 text-orange-500 border border-orange-500/15"
                          >
                            {factor}
                          </span>
                        ))}

    ...
    <div className="h-full flex flex-col">
        ...
        <div className={`${styles.headerBg} border ${styles.headerBorder} rounded-xl p-3 sm:p-4`}>
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-[10px] text-text-tertiary">{markets.length} active market{markets.length !== 1 ? 's' : ''}</span>
              </div>
            </div>
          </div>
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full pl-9 pr-10 py-2.5 bg-surface-elevated/50 border border-border-subtle rounded-lg text-sm text-text-primary placeholder:text-text-tertiary focus:outline-none focus:border-accent-500/50 transition-colors"
              style={{ fontSize: '16px' }}
            />
            {isSearching && <Loader2 className="absolute right-3 top-1/2 -translate-y-1/2 w-4 h-4 text-accent-500 animate-spin" />}
          </div>
      ...
      <div className="flex-1 overflow-y-auto px-3 sm:px-4 pt-3 pb-4 space-y-3">
        {sentiment && !searchQuery && (
          <div className={`${styles.inputSolidBg} border ${styles.inputBorder} rounded-xl p-4 ${SENTIMENT_CONFIG[sentiment.overallSentiment]?.glow ?? ''}`}>
            <div className="flex items-center gap-2.5 mb-2">
              <div className={`w-6 h-6 rounded-lg flex items-center justify-center ${
                sentiment.overallSentiment === 'bullish' ? 'bg-green-500/15' :
                sentiment.overallSentiment === 'bearish' ? 'bg-red-500/15' :
                sentiment.overallSentiment === 'mixed' ? 'bg-yellow-500/15' : 'bg-surface-elevated'
              }`}>
                <Activity className={`w-3.5 h-3.5 ${SENTIMENT_CONFIG[sentiment.overallSentiment]?.text ?? 'text-text-secondary'}`} />
              </div>
              <span className="text-sm font-semibold text-text-primary">
                {activeTag}
                <span className={`ml-1.5 ${SENTIMENT_CONFIG[sentiment.overallSentiment]?.text ?? ''}`}>
                  {SENTIMENT_CONFIG[sentiment.overallSentiment]?.label ?? sentiment.overallSentiment}
                </span>
              </span>
            </div>
            <p className="text-xs text-text-secondary leading-relaxed ml-8.5">{sentiment.sentimentSummary}</p>
          </div>
        )}
        ...
        {isLoading ? (
          ...
          <div className="space-y-2">
            {markets.map((market) => {
              ...
              return (
                <div key={market.id} className="rounded-xl border border-border-subtle bg-surface-elevated/30 p-4 transition-all duration-200 hover:border-accent-500/20">
                    ...
                    <div className="space-y-2 mb-3">
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          <span className="text-[10px] font-semibold text-green-400 uppercase tracking-wider">Yes</span>
                      ...
                      {market.outcomes.map((outcome) => (
                        <div key={outcome.id} className="flex items-center justify-between py-1">
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            <span className="text-xs font-semibold text-text-primary w-8 text-right">{Math.round(outcome.probability)}%</span>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="flex items-center gap-4 text-[10px] text-text-tertiary pt-2 border-t border-border-subtle/50">
                    ...
                    {market.tags.length > 0 && (
                      <div className="flex gap-1 ml-auto">
                        {market.tags.slice(0, 3).map((tag) => (
                          <span key={tag} className="px-1.5 py-0.5 rounded text-[9px] font-medium bg-surface-elevated text-text-tertiary border border-border-subtle/50">
                            {tag}
                          </span>
                        ))}
                      </div>
